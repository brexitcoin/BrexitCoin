...
{
    // Convert brexit:// to brexit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("brexit://"))
    {
        uri.replace(0, 12, "brexit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Brexit.lnk";
}
...
{
    return GetAutostartDir() / "brexit.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Brexit-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  brexit-qt [" + tr("command-line options") + "]                     " + "\n";
